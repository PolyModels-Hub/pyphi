import numpy as np
from bokeh.io import show, output_file as _bokeh_output_file, output_notebook
from bokeh.plotting import figure
from bokeh.layouts import column
from bokeh.models import ColumnDataSource,LabelSet,Span,Legend
import pandas as pd

# Import required functions from sibling modules to avoid circular import
from .pca import pca_pred, hott2
from .pls import pls_pred, spe
from .diagnostics import contributions
from .utils import unique, single_score_conf_int
from ._internal import scores_conf_int_calc

# Create a namespace object so existing code using phi.function() still works
class _PhiNamespace:
    """Namespace to maintain backward compatibility with phi.function() calls."""
    pca_pred = staticmethod(pca_pred)
    pls_pred = staticmethod(pls_pred)
    hott2 = staticmethod(hott2)
    spe = staticmethod(spe)
    contributions = staticmethod(contributions)
    unique = staticmethod(unique)
    single_score_conf_int = staticmethod(single_score_conf_int)
    scores_conf_int_calc = staticmethod(scores_conf_int_calc)
...
# =============================================================================
# Output routing helpers (file vs. inline notebook)
# =============================================================================

_USE_NOTEBOOK_OUTPUT = False


def enable_notebook_output() -> None:
    """Render plots inline in a Jupyter notebook instead of writing HTML files.

    Call this once near the top of a notebook:

    >>> from pyphi import plots as pplots
    >>> pplots.enable_notebook_output()

    After that, all plotting functions will just `show(...)` inline in the
    notebook without creating or opening standalone HTML files.
    """
    global _USE_NOTEBOOK_OUTPUT
    _USE_NOTEBOOK_OUTPUT = True
    output_notebook()


def _output_file(*args, **kwargs):
    """Internal wrapper around Bokeh's output_file.

    When notebook mode is enabled, this becomes a no-op so that plots render
    inline instead of being written to disk and opened in a browser tab.
    """
    if _USE_NOTEBOOK_OUTPUT:
        return
    return _bokeh_output_file(*args, **kwargs)
...
def r2pv(mvm_obj,*,plotwidth=600,plotheight=400,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        ...
        rnd_num=timestr()
        _output_file("r2xypv_"+rnd_num+".html",title="R2"+ yaxlbl+ "YPV",mode='inline') 
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("r2xpv_"+rnd_num+".html",title='R2XPV',mode='inline') 
        #colormap =cm.get_cmap("rainbow")
        colormap = matplotlib.colormaps['rainbow']
        different_colors=A
        color_mapping=colormap(np.linspace(0,1,different_colors),1,True)
...
def loadings(mvm_obj,*,plotwidth=600,xgrid=False,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings "+space_lbl+" Space_"+rnd_num+".html",title=space_lbl+' Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings Y Space_"+rnd_num+".html",title='Y Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings X Space_"+rnd_num+".html",title='X Loadings PCA',mode='inline') 
        for i in list(np.arange(A)):
...
def loadings_map(mvm_obj,dims,*,plotwidth=600,addtitle='',material=False,zspace=False,textalpha=0.75):
    ...
    if 'Q' in mvmobj:
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings Map"+rnd_num+".html",title='Loadings Map',mode='inline')
...
def weighted_loadings(mvm_obj,*,plotwidth=600,xgrid=False,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings "+space_lbl+" Space_"+rnd_num+".html",title=space_lbl+' Weighted Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings Y Space_"+rnd_num+".html",title='Y Weighted Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Loadings X Space_"+rnd_num+".html",title='X Weighted Loadings PCA',mode='inline') 
        for i in list(np.arange(A)):
...
def vip(mvm_obj,*,plotwidth=600,material=False,zspace=False,addtitle=''):
    ...
    if 'Q' in mvmobj:  
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("VIP_"+rnd_num+".html",title='VIP Coefficient',mode='inline') 
...
def score_scatter(mvm_obj,xydim,*,CLASSID=False,colorby=False,Xnew=False,
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Score_Scatter_"+rnd_num+".html",title='Score Scatter t['+str(xydim[0])+'] - t['+str(xydim[1])+ ']',mode='inline')
...
def score_line(mvmobj,dim,*,CLASSID=False,colorby=False,Xnew=False,add_ci=False,add_labels=False,add_legend=True,plotline=True,plotwidth=600,plotheight=600):
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("Score_Line_"+rnd_num+".html",title='Score Line t['+str(dim[0])+ ']',mode='inline')
...
def diagnostics(mvmobj,*,Xnew=False,Ynew=False,score_plot_xydim=False,plotwidth=600,ht2_logscale=False,spe_logscale=False):
    ...
    #rnd_num=str(int(np.round(1000*np.random.random_sample())))      
    rnd_num=timestr()         
    _output_file("Diagnostics"+rnd_num+".html",title='Diagnostics',mode='inline') 
...
def predvsobs(mvmobj,X,Y,*,CLASSID=False,colorby=False,x_space=False):
   ...
   (sgarciam@ic.ac.uk ,salvadorgarciamunoz@gmail.com)
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("ObsvsPred_"+rnd_num+".html",title='ObsvsPred',mode='inline')
        plot_counter=0
...
contributions_plot(mvmobj,X,cont_type,*,Y=False,from_obs=False,to_obs=False,lv_space=False,plotwidth=800,plotheight=600,xgrid=False):
    ...
    #rnd_num=str(int(np.round(1000*np.random.random_sample())))
    rnd_num=timestr()
    _output_file("Contributions"+rnd_num+".html",title='Contributions',mode='inline')
...
def mb_weights(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    for i in list(np.arange(A)):
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("blockweights_"+rnd_num+".html",title="Block Weights",mode='inline')         
...
def mb_r2pb(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    for i in list(np.arange(A)):
        ...
        #rnd_num=str(int(np.round(1000*np.random.random_sample())))
        rnd_num=timestr()
        _output_file("r2perblock"+rnd_num+".html",title="R2 per Block",mode='inline') 
        #colormap =cm.get_cmap("rainbow")
        colormap = matplotlib.colormaps['rainbow']
        different_colors=A
        color_mapping=colormap(np.linspace(0,1,different_colors),1,True)
...
def mb_vip(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    #rnd_num=str(int(np.round(1000*np.random.random_sample())))
    rnd_num=timestr()
    _output_file("blockvip"+rnd_num+".html",title="Block VIP",mode='inline') 
...
def barplot(yheights,xtick_labels,*,plotwidth=600,plotheight=600,
    ...
    """
    rnd_num=timestr()
    _output_file("BarPlot_"+rnd_num+".html",title=tabtitle,mode='inline') 
...
def lineplot(X,*,ids_2_include=False,x_axis=False,plot_title='Main Title',tab_title='Tab Title',
   ...
   Optional Parameters:    
    ...
    if isinstance(X,pd.DataFrame): #Common x-axis
        ...
        rnd_num=timestr()          
        _output_file("LinePlot"+rnd_num+".html",title=tab_title,mode='inline')
        if individual_plots:
...
def plot_spectra(X,*,xaxis=False,plot_title='Main Title',tab_title='Tab Title',
    ...
    #rnd_num=str(int(np.round(1000*np.random.random_sample())))
    rnd_num=timestr()                
    _output_file("Spectra"+rnd_num+".html",title=tab_title,mode='inline')
...
def scatter_with_labels(x,y,*,xlabel='X var',ylabel='YVar',labels=False,
                        ...
                        fill_alpha =0.2,line_alpha=0.4):
    rnd_num=timestr()
    _output_file("Scatter Plot"+rnd_num+".html",title=tabtitle,mode='inline')    
    x_p = x
    y_p = y                        
    TOOLS = "save,wheel_zoom,box_zoom,pan,reset,box_select,lasso_select"
    TOOLTIPS = [
